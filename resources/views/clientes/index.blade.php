...
<head>
    ...
    <!-- Usamos Tailwind CSS como placeholder para el template Gentelella Alela -->
    ...
    <div class="container mx-auto px-4 py-8 max-w-4xl bg-white shadow-xl rounded-2xl">
        ...
        <div class="mb-6 text-center">
            ...
            <p class="text-gray-500 mt-2">
                Añade, edita y elimina clientes de tu base de datos contable.
            </p>
        </div>
        ...
        <div class="bg-gray-100 p-6 rounded-xl shadow-inner mb-8">
            <h2 class="text-xl font-semibold mb-4 text-gray-700">Añadir Nuevo Cliente</h2>
            <form action="{{ route('clientes.store') }}" method="POST">
                ...
                <div class="mt-6 flex justify-end">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-6 rounded-lg shadow-md transition-colors">Guardar Cliente</button>
                </div>
            </form>
        </div>

        <!-- Lista de clientes -->
        <h2 class="text-xl font-semibold mb-4 text-gray-700">Lista de Clientes</h2>
        <div class="space-y-4">
            @foreach ($clientes as $cliente)
                <div class="bg-gray-50 p-4 rounded-lg shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between transition-shadow hover:shadow-md">
                    <div class="mb-2 sm:mb-0">
                        ...
                        <p class="text-sm text-gray-500">Contacto: {{ $cliente->contacto }}</p>
                        <p class="text-sm text-gray-500">Teléfono: {{ $cliente->telefono }}</p>
                        <p class="text-sm text-gray-500">Dirección: {{ $cliente->direccion }}</p>
                    </div>
                    <div class="flex space-x-2 mt-2 sm:mt-0">
                        <!-- Botón de Editar -->
                        <a href="{{ route('clientes.edit', $cliente->id) }}" class="bg-yellow-400 text-gray-800 font-medium py-1 px-3 rounded-md hover:bg-yellow-500 transition-colors">Editar</a>
                        <!-- Botón de Eliminar -->
                        <form action="{{ route('clientes.destroy', $cliente->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-500 text-white font-medium py-1 px-3 rounded-md hover:bg-red-600 transition-colors" onclick="return confirm('¿Estás seguro de que quieres eliminar a este cliente?');">Eliminar</button>
                        </form>
                    </div>
                </div>
            @endforeach
